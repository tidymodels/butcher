test_that("flexsurvreg + predict() works", {
  ...
  x <- axe_env(fit)
  expect_identical(
    attr(attributes(x$data$m)$terms, ".Environment"),
    rlang::base_env()
  )
...
test_that("flexsurvreg markov + predict() works", {
  ...
  suppressPackageStartupMessages(library(flexsurv))
  fit <- flexsurvreg(
    Surv(years, status) ~ trans + shape(trans),
    data = bosms3,
    dist = "weibull"
  )
  x <- axe_call(fit)
  ...
  expect_identical(attr(x$concat.formula, ".Environment"), rlang::base_env())
  expect_identical(
    attr(x$all.formulae$scale, ".Environment"),
    rlang::base_env()
  )
  x <- butcher(fit)
  ...
  expect_equal(
    flexsurv::msfit.flexsurvreg(x, t = tgrid, trans = tmat)$Haz$Haz[1:3],
    expected_output
  )
