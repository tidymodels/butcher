...
Here we assign the current blob object `x` to the variable `old` as a means to evaluate the memory released once `axe_call` is executed on the original model object. We also include `assess_object` with the additional string parameter of `disabled` so console messages will be printed out, alerting users of any downstream functions would be affected by axing the call. Since the original model object is fundamentally different, we attach an additional `butcher_blob` class by including `add_butcher_class` at the end of each axe method. Once the axe methods are set, we then have a skeleton file `tests/testthat/test-blob.R` to aid in any unit testing. 
