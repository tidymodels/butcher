...
#' @rdname axe-spark
#' @export
axe_fitted.ml_model <- function(x, ...) {
...
axe_env.ml_pipeline_model <- function(x, ...) {
  ...
  add_butcher_class(x) # note, loss of pretty output
}
...
#' @rdname axe-spark
#' @export
axe_env.ml_pipeline <- function(x, ...) {
  x$stages <- purrr::map(x$stages, function(z) axe_env(z, ...))
  add_butcher_class(x) # note, loss of pretty output
}
