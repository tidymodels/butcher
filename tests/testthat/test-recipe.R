...
impute_empty_env <- function(axed, step_number) {
  expect_identical(
    attr(axed$steps[[step_number]]$impute_with[[1]], ".Environment"),
    rlang::base_env()
  )
}
...
inputs_empty_env <- function(axed, input_number) {
  expect_identical(
    attr(axed$steps[[1]]$input[[input_number]], ".Environment"),
    rlang::base_env()
  )
}
...
test_that("recipe + step_novel + axe_env() works", {
  ...
  sacr_te <- Sacramento[501:nrow(Sacramento), ] |>
...
test_that("recipe + step_ordinalscore + axe_env() works", {
  ...
  ord_data <-
    data.frame(
      ...
      fail_severity = factor(fail_lvls, levels = fail_lvls, ordered = TRUE)
    )
...
test_that("recipe + step_ratio + axe_env() works", {
  ...
  biomass_tr <- biomass[biomass$dataset == "Training", ]
  rec <- recipe(
    HHV ~ carbon + hydrogen + oxygen + nitrogen + sulfur + total,
    data = biomass_tr
  ) |>
...
test_that("recipe + step_lag + axe_env() works", {
  df <- data.frame(x = runif(20), index = 1:20)
  rec <- recipe(~., data = df) |>
